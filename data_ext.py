...
def _maybe_first_or_second(o, i=0): return o[i] if isinstance(o, tuple) else o
...
class SequentialDataLoader(TfmdDL):
    ...
    def __init__(self, dataset, lens=None, cache=2, bs=64, seq_len=72, num_workers=0, **kwargs):
        self.items = ReindexCollection(dataset, cache=cache, tfm=partial(_maybe_first_or_second, i=0))
        self.items_y = ReindexCollection(dataset, cache=cache, tfm=partial(_maybe_first_or_second, i=1))
...
class SequentialTextBlock(TransformBlock):
    ...
    def from_df(cls, text_cols, vocab=None, seq_len=72, bmin_freq=3, max_vocab=60000, **kwargs):
        "Build a `TextBlock` from a dataframe using `text_cols`"
        return cls(Tokenizer.from_df(text_cols, **kwargs), vocab=vocab,
                   seq_len=seq_len,
                   min_freq=min_freq, max_vocab=max_vocab)

    @classmethod
    @delegates(Tokenizer.from_folder, keep=True)
    def from_folder(cls, path, vocab=None, seq_len=72, min_freq=3, max_vocab=60000, **kwargs):
        "Build a `TextBlock` from a `path`"
        return cls(Tokenizer.from_folder(path, **kwargs), vocab=vocab, seq_len=seq_len,
                   min_freq=min_freq, max_vocab=max_vocab)
